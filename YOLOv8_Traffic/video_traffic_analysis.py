...
def process_video(video_path, model_path='yolov8n.pt', output_dir='traffic_output'):
    """Process video with vehicle tracking and counting, save frames instead of displaying"""
    # Initialize tracker
    ...
    print(f"Processing video and saving frames to: {output_dir}")
    print("Press Ctrl+C to stop processing")
    
    frame_count = 0
    save_interval = max(1, int(fps // 2))  # Save every 0.5 seconds
    ...
    try:
        while True:
            ...
            if frame_count % save_interval == 0:
                output_path = os.path.join(output_dir, f"traffic_frame_{frame_count:06d}.png")
            ...
            if frame_count % 100 == 0:
                ...
                print(f"Progress: {progress:.1f}% - Vehicles counted: {tracker.vehicle_count}")
